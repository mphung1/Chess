...
export default function Page({ params }: { params: { roomId: string } }) {
  ...
  const [selectedSquare, setSelectedSquare] = useState<PieceCoordinate | null>(
    null
  );
  ...
  useEffect(() => {
    ...
    protoPlayer.setColor(
      player.color == Color.WHITE ? ProtoColor.WHITE : ProtoColor.BLACK
    );
    ...
    st.on("data", (res) => {
      ...
      if (!player2Ready && player && !move) {
        ...
        chess.addPlayer(
          new Player(player.getName(), getColorName(player.getColor()))
        );
      ...
      if (player && move) {
        ...
        if (!from || !to) return;
        movePiece(
          new PieceCoordinate(from.getX(), from.getY()),
          new PieceCoordinate(to.getX(), to.getY())
        );
      }
    });
    st.on("error", (err) => {
  ...
  useEffect(() => {
    ...
    client.getRoomInfo(roomRequest).then((res) => {
      ...
      if (player1?.getName() && !players[0]) {
        ...
        chess.addPlayer(
          new Player(player1.getName(), getColorName(player1.getColor()))
        );
      ...
      if (player2?.getName() && !players[1]) {
        ...
        chess.addPlayer(
          new Player(player2.getName(), getColorName(player2.getColor()))
        );
        setPlayer2Ready(true);
        chess.playerTurn = Color.WHITE;
        chess.gameStatus = GameStatus.IN_PROGRESS;
  ...
  const updateBoard = (coordinate: PieceCoordinate) => {
    ...
    if (
      !selectedSquare &&
      squares[coordinate.x][coordinate.y].piece &&
      chess.playerTurn == squares[coordinate.x][coordinate.y].piece?.color
    ) {
    ...
    if (
      selectedSquare?.x == coordinate.x &&
      selectedSquare?.y == coordinate.y
    ) {
    ...
    if (
      selectedSquare &&
      squares[selectedSquare.x][selectedSquare.y].piece?.color ==
        squares[coordinate.x][coordinate.y].piece?.color
    ) {
  ...
  const isSelected = (coordinate: PieceCoordinate) => {
    return (
      selectedSquare?.x == coordinate.x && selectedSquare?.y == coordinate.y
    );
  };
              ...
              <div key={`x-${x}`} className="flex flex-row">
                {col.map((y) => (
                  <div key={`y-${y}`} className="flex flex-col">
                    <div
                      className={clsx("chess-cell", {
                        "bg-white": (x + y) % 2 === 0,
                        "bg-slate-400": (x + y) % 2 !== 0,
                      })}
                    >
                      <SquareComponent
                        isPossibleMove={isPossibleMove(
                          new PieceCoordinate(x, y)
                        )}
                        isSelected={isSelected(new PieceCoordinate(x, y))}
                        piece={chess.board.squares[x][y].piece}
                        onClick={() => updateBoard(new PieceCoordinate(x, y))}
                      />
                    </div>
                  </div>
                ))}
